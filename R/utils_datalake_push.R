...
#' Check if all files are images (PNG, JPG, JPEG)
#' @param files Vector of file paths
...
is_html_dataset <- function(files) {
  ...
  # Check if all files are HTML
  all(extensions %in% html_extensions)
}
...
collect_all_metadata_interactive <- function(files = NULL) {
  ...
  # Check if dealing with streamlined metadata content (images or HTML)
  is_images <- !is.null(files) && is_image_dataset(files)
  is_html <- !is.null(files) && is_html_dataset(files)
  use_streamlined <- is_images || is_html
  
  if (is_images) {
    cli::cli_text("🖼️ Images détectées - métadonnées simplifiées")
  } else if (is_html) {
    cli::cli_text("📄 Fichiers HTML détectés - métadonnées simplifiées")
  } else {
  ...
  metadata <- collect_required_system_metadata(metadata, use_streamlined)
...
collect_required_system_metadata <- function(metadata, use_streamlined = FALSE) {
  if (use_streamlined) {
    ...
    cli::cli_h3("📝 Métadonnées système simplifiées")
...
prepare_s3_metadata <- function(custom_metadata) {
  ...
  # Default governance values for non-ethical datasets
  s3_metadata[["consent_expiry_date"]] <- "9999-12-31"
  s3_metadata[["data_destruction_date"]] <- "9999-12-31"
  s3_metadata[["sensitivity_level"]] <- "1"
  ...
  tryCatch(
    {
      ...
      if (success) {
        ...
        cli::cli_alert_success("🔄 Indexation déclenchée...")
      } else {
        logger::log_error(
...
handle_navigation_choice <- function(choice, current_dir) {
  ...
  if (choice == "p") {
    ...
    if (parent_dir != current_dir) {
      ...
      list(action = "update_dir", path = parent_dir)
    } else {
  ...
  if (choice == "h") {
    ...
    list(action = "update_dir", path = home_dir)
  }

  if (choice == ".") {
    if (confirm_simple_directory_selection(current_dir)) {
      list(action = "select", path = current_dir)
    } else {
      list(action = "continue")
    }
  }
...
confirm_directory_selection <- function(dir_path) {
  ...
  if (length(supported_files) == 0) {
    cli::cli_alert_warning("⚠️ Aucun fichier supporté trouvé dans ce dossier")
    ask_yes_no("Voulez-vous quand même sélectionner ce dossier?")
  }
...
format_file_size <- function(size_bytes) {
  ...
  if (size_bytes < 1024) {
    ...
    paste(round(size_bytes / 1024^3, 2), "GB")
  }
}
